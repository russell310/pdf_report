...
pdfmetrics.registerFont(TTFont('rajdhani', 'Rajdhani-Regular.ttf'))
pdfmetrics.registerFont(TTFont('rajdhaniBold', 'Rajdhani-Bold.ttf'))
pdfmetrics.registerFont(TTFont('exoBold', 'Exo-SemiBold.ttf'))
...
class PdfPrint:
    ...
    def line_chart_draw(self, values, days):
        ...
        # set font desired
        chart.lineLabels.fontName = 'rajdhani'
        ...
        chart.categoryAxis.labels.fontName = 'rajdhani'
        if nr_days > 7:
            chart.categoryAxis.labels.angle = 45
            chart.categoryAxis.labels.boxAnchor = 'e'
        chart.categoryAxis.categoryNames = days
        # change y axe format
        chart.valueAxis.labels.fontName = 'rajdhani'
        chart.valueAxis.labelTextFormat = '%2.0f °C'
        chart.valueAxis.valueStep = 10
        if min_temp > 0:
            chart.valueAxis.valueMin = 0
        llabels = ['Max temp', 'Min temp']
        d.add(self.title_draw(250, 180, ('Temperatures statistics')))
        d.add(chart)
    ...
    def pie_chart_draw(self, values, llabels):
        ...
        pc.slices.fontName = 'rajdhani'
        d.add(self.title_draw(250, 180, ('Precipitation probability statistics')))
        d.add(pc)
    ...
    def vertical_bar_chart_draw(self, values, days, llabels):
        ...
        bc.categoryAxis.labels.fontName = 'rajdhani'

        # set label format for each bar
        bc.barLabelFormat = '%d'
        bc.barLabels.fontName = 'rajdhani'
        ...
        bc.valueAxis.labels.fontName = 'rajdhani'
        bc.valueAxis.valueMin = 0

        d.add(self.title_draw(250, 190, ('Wind speed statistics')))
        d.add(bc)
    ...
    def report(self, weather_history, title):
        ...
        styles.add(ParagraphStyle(
            name="TableHeader", fontSize=11, alignment=TA_CENTER, fontName="exoBold"))
        styles.add(ParagraphStyle(
        ...
        header_data = []
        header_data.append([
            self.get_image(os.path.join(settings.BASE_DIR, 'static/images/logo.png'), 50),
            Paragraph('AR Integration & Soltution<br/>Dhaka<br/>Bangladesh', styles['ParagraphTitle']),
        ])
        hd_table = Table(header_data, colWidths=[doc.width / 2.0] * 2)
        hd_table.hAlign = 'LEFT'
        hd_table.setStyle(TableStyle([('VALIGN', (0, 0), (-1, 0), 'MIDDLE')]))
        ...
        for wh in weather_history:
            ...
            # add a row to table
            table_data.append(
                [Paragraph(str(wh.date), styles['Justify']),
                 Paragraph(wh.town.name, styles['Justify']),
                 Paragraph("{0} °C".format(wh.max_temperature), styles['Justify']),
                 Paragraph("{0} °C".format(wh.min_temperature), styles['Justify']),
                 Paragraph("{0} km/h".format(wh.wind_speed), styles['Justify']),
                 Paragraph("{0} mm".format(wh.precipitation), styles['Justify']),
                 Paragraph("{0} %".format(wh.precipitation_probability), styles['Justify'])])
        # create table
